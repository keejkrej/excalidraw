...
export const ExcalidrawAPIContext = React.createContext<ExcalidrawImperativeAPI | null>(null);
...
class App extends React.Component<AppProps, AppState> {
  ...
  public onEvent = this.editorLifecycleEvents.on.bind(this.editorLifecycleEvents) as AppEventBus<
    ExcalidrawImperativeAPIEventMap,
    typeof editorLifecycleEventBehavior
  >["on"];
  ...
  private previousHoveredBindableElement: NonDeletedExcalidrawElement | null = null;
  ...
  private isDoubleClick = (
    lastPointerEvent: PointerEvent | React.PointerEvent<HTMLElement> | undefined | null,
    currentPointerEvent: PointerEvent | React.PointerEvent<HTMLElement>,
  ) => {
    return (
      lastPointerEvent != null &&
      currentPointerEvent.timeStamp - lastPointerEvent.timeStamp <= TAP_TWICE_TIMEOUT
    );
  };
  ...
  private onKeyDown = withBatchedUpdates((event: React.KeyboardEvent | KeyboardEvent) => {
    ...
    if (!shouldPreventToolSwitching && this.state.viewModeEnabled && event.key === KEYS.ESCAPE) {
    ...
    if (isArrowKey(event.key)) {
      ...
      selectedElements
        .filter((el): el is NonDeleted<ExcalidrawArrowElement> => isBindingElement(el))
        .filter((arrow) => {
    ...
    const isPickingStroke = lowerCased === KEYS.S && event.shiftKey && !event[KEYS.CTRL_OR_CMD];
          ...
          if (element.startBinding) {
            this.scene.mutateElement(element, {
              startBinding: {
                ...
                ...calculateFixedPointForNonElbowArrowBinding(
                  element,
                  elementsMap.get(element.startBinding.elementId) as ExcalidrawBindableElement,
                  "start",
                  elementsMap,
                ),
              },
          ...
          if (element.endBinding) {
            this.scene.mutateElement(element, {
              endBinding: {
                ...
                ...calculateFixedPointForNonElbowArrowBinding(
                  element,
                  elementsMap.get(element.endBinding.elementId) as ExcalidrawBindableElement,
                  "end",
                  elementsMap,
                ),
              },
  ...
  private getSelectedTextElement(
    ...
    return getBoundTextElement(selectedElement, this.scene.getNonDeletedElementsMap());
  }
  ...
  private isHittingTextAutoResizeHandle = (
    ...
    const activeTextElement = getActiveTextElement(selectedElements, this.state);
  ...
  private startTextEditing = ({
    ...
    const existingTextElement =
      this.getSelectedTextElement(container) || this.getTextElementAtPosition(sceneX, sceneY);
  ...
  private handleElementLinkClick = (event: React.PointerEvent<HTMLCanvasElement>) => {
    const draggedDistance = pointDistance(
  ...
  private handleCanvasPointerDown = (event: React.PointerEvent<HTMLElement>) => {
    ...
    if (this.state.activeTool.type === "lasso") {
      ...
      const shouldForceLassoReselect =
        event.altKey && event[KEYS.CTRL_OR_CMD] && !pointerDownState.resize.handleType;
  ...
  private isHittingCommonBoundingBoxOfSelectedElements(
    ...
    const boundsPadding = (DEFAULT_TRANSFORM_HANDLE_SPACING * 2) / this.state.zoom.value;
  ...
  private onPointerUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      const selectedTextEditingContainer = this.getSelectedTextEditingContainerAtPosition(
        hitElement,
        sceneCoords,
      );
      ...
      if (isBindingElement(element)) {
        ...
        element.boundElements?.forEach((boundElement) => {
          if (boundElement.type === "arrow") {
            const arrow = this.scene.getElement(boundElement.id) as ExcalidrawArrowElement;
  ...
  loadFileToCanvas = async (file: File, fileHandle: FileSystemFileHandle | null) => {
