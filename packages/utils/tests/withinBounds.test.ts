...
const makeBBox = (minX: number, minY: number, maxX: number, maxY: number): Bounds => [
  minX,
  minY,
  maxX,
  maxY,
...
describe("elementPartiallyOverlapsWithOrContainsBBox()", () => {
  it("should return true if element overlaps, is inside, or contains", () => {
    ...
    // bbox contains element
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(0, 0, 100, 100), bbox)).toBe(
      true,
    );
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(10, 10, 90, 90), bbox)).toBe(
      true,
    );

    // element contains bbox
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(-10, -10, 110, 110), bbox)).toBe(
      true,
    );

    // element overlaps bbox from top-left
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(-10, -10, 100, 100), bbox)).toBe(
      true,
    );
    // element overlaps bbox from top-right
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(90, -10, 100, 100), bbox)).toBe(
      true,
    );
    // element overlaps bbox from bottom-left
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(-10, 90, 100, 100), bbox)).toBe(
      true,
    );
    // element overlaps bbox from bottom-right
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(90, 90, 100, 100), bbox)).toBe(
      true,
    );
  });
  ...
  it("should return false if element does not overlap", () => {
    ...
    // outside diagonally
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(110, 110, 100, 100), bbox)).toBe(
      false,
    );

    // outside on the left
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(-110, 10, 50, 50), bbox)).toBe(
      false,
    );
    // outside on the right
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(110, 10, 50, 50), bbox)).toBe(
    ...
    // outside on the top
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(10, -110, 50, 50), bbox)).toBe(
      false,
    );
    // outside on the bottom
    expect(elementPartiallyOverlapsWithOrContainsBBox(makeElement(10, 110, 50, 50), bbox)).toBe(
